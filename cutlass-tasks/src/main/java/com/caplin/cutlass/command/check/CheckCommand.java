...
public class CheckCommand extends AbstractPlugin implements LegacyCommandPlugin
{
	...
	public int doCommand(String... args) throws CommandArgumentsException, CommandOperationException
	{
		...
		List<File> sdkLibsApplicationJars = Arrays.asList(BRJSModelAccessor.root.appJars().dir().listFiles(CAPLIN_JAR_FILE_NAME_FILTER));
		List<SdkJsLib> sdkThirdpartyLibraries = BRJSModelAccessor.root.sdkLibs();
		
		for(App application : BRJSModelAccessor.root.userApps())
		{
