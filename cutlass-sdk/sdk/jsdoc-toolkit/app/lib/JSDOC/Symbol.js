...
JSDOC.Symbol.prototype.setTags = function() {
	...
	// @alpha
	var isAlpha = this.comment.getTag("alpha");
	this.isAlpha = (isAlpha.length > 0);

	// @beta
	var isBeta = this.comment.getTag("beta");
	this.isBeta = (isBeta.length > 0);

	// @deprecated
	var deprecated = this.comment.getTag("deprecated");
	this.isDeprecated = (deprecated.length > 0);
	...
	*/
	...
	this.implement = this.comment.getTag("implements");
	...
	*/
	...
	var interfaceTag = this.comment.getTag("interface");
	var isInterface = (interfaceTag.length);

	var singletonTag = this.comment.getTag("singleton");
	var isSingleton = (singletonTag.length);
	
	var packageTag = this.comment.getTag("package");
	var isPackage = (packageTag.length);
	
	if (this.isBuiltin()) 
	{
		this.classType += "Built-In ";
	}	
	...
	{
		this.classType += "Singleton ";
	}
	else if (isPackage || this.isNamespace) 
	{
		if (this.is('FUNCTION')) {
			this.classType += "Function ";
		}
		this.classType += "Package	 ";
	}
	else {
