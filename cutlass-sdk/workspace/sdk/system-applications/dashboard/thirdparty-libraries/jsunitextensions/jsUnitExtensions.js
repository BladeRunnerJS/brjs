...
function assertException(sMsg, fExpectedException, fCode)
{
	if (!_isMessageSupplied(arguments, 3)) return _reinvokeWithMessage(arguments);
   ...
   var bFail = false;
    ...
    {
        assertTrue(sMsg + ": the exception was not of the expected type" , e instanceof fExpectedException);
    }
    if(bFail) fail(sMsg + ": an exception was expected but one did not occur");
};
