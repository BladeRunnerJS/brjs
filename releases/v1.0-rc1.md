...
Other than the above, 1.0-RC1 is backwardsly compatible with v0.15.4.
...
HTML templates can now also be embedded in `index.html` rather than loading them via seperate XHR request. To do this include the `<@html.bundle@/>` in the `<head>` of the page and continue using the HTML service as before.

Existing apps and templates will still be compatible with these changes. See https://github.com/BladeRunnerJS/brjs/pull/1338 & https://github.com/BladeRunnerJS/brjs/pull/1352 for more details.
