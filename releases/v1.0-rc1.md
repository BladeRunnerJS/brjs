...
- The XML namespaces used for XML config files have changed. Any existing aliases.xml, aliasDefinitions.xml or bundleConfig.xml files with need to change to be compatible with the 1.0-RC1 changes. See below for more information.
...
The XML namespaces used for BladeRunnerJS XML config has changed to use the `schema.bladerunnerjs.org` domain. The old and new namespaces are shown below. All instances of the previous namespaces should be replaced with the new namespace.
