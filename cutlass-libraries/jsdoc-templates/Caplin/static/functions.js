$(function () {
	...
	var nodesToOpenInitially = $.map($(".classDepth1"), function(n) { return n.id }) 
	var classId = getParentNavClassIdFromUrl(window.location.href);
	if (classId != "")
	{
		nodesToOpenInitially.push(classId);
	}
	...
	$("#api_tree").jstree({
		...
		"core" : { 
			"initially_open" : nodesToOpenInitially
		}
	}).bind("before.jstree", function (e, data) {
        if(data.func === "close_node") {
        	if ( $(data.args[0]).parent().parent().attr("id") == "api_tree" )
        	{
