...
function loadHtml(url) {
	...
	var templatesLoadedIndicatorId = 'brjs-html-templates-loaded';
	var templatesLoadedIndicator = elHead.querySelector('template#'+templatesLoadedIndicatorId);
	...
	if (!templatesLoadedIndicator) {
		...
		if (!templatesInlineIndicator) {
			...
			while (templateContainerChildNodes.length > 0) {
				elHead.appendChild( templateContainerChildNodes[0] );
			}
		}
		...
		templatesLoadedIndicator = document.createElement("template");
		templatesLoadedIndicator.id = templatesLoadedIndicatorId;
		elHead.appendChild(templatesLoadedIndicator);
	}
}
