		...
		if (symbol.memberOf) {
			...
			if (!container) {
				if (JSDOC.Lang.isBuiltin(symbol.memberOf)) container = JSDOC.Parser.addBuiltin(symbol.memberOf);
					...
					// Inner functions from private classes that were not marked private were showing WARNING, preventing
					// Any none classes from packages which don't have a package description eg. caplin.lib.package.class throw WARNING, preventing as they are considered static
					if (!symbol.isStatic && !symbol.isInner)
...
JSDOC.SymbolSet.prototype.resolveAugments = function() {
	this.resolveSymbol("augments");
}

JSDOC.SymbolSet.prototype.resolveImplements = function() {
	this.resolveSymbol("implement");
};

JSDOC.SymbolSet.prototype.resolveSymbol = function(sSymbolType) {
	for (var p = this._index.first(); p; p = this._index.next()) {
		var symbol = p.value;
		
		if (symbol.alias == "_global_" || symbol.is("FILE")) continue;
		JSDOC.SymbolSet.prototype.walk.apply(this, [symbol, sSymbolType]);
	}
}

JSDOC.SymbolSet.prototype.walk = function(symbol, sSymbolType) {
	
	var augments = symbol[sSymbolType];
	...
	for(var i = 0; i < augments.length; i++) {
		...
		if (contributer) {			
			if (contributer[sSymbolType].length) {
		...
		else LOG.warn("Can't augment contributer: "+augments[i]+", not found inside " + symbol.alias + ".");
	}
}
