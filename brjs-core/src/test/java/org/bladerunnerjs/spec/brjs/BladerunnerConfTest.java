...
public class BladerunnerConfTest extends SpecTest {
	...
	public void readingBladerunnerConfWithMissingLoginReamlThrowsException() throws Exception {
		given(brjs).containsFileWithContents("conf/bladerunner.conf", "defaultInputEncoding: UTF-8\ndefaultOutputEncoding: UTF-8\njettyPort: 7070");
	...
	public void bladerunnerConfThatAlreadyExistsCanBeReadAndModified() throws Exception {
		given(brjs).containsFileWithContents("conf/bladerunner.conf", "defaultInputEncoding: UTF-8\ndefaultOutputEncoding: UTF-8\njettyPort: 7070\nloginRealm: BladeRunnerLoginRealm");
		when(brjs).bladerunnerConf().setJettyPort(8888).setDefaultInputEncoding("ISO-8859-1").write();
		then(brjs).fileHasContents("conf/bladerunner.conf", "defaultInputEncoding: ISO-8859-1\ndefaultOutputEncoding: UTF-8\njettyPort: 8888\nloginRealm: BladeRunnerLoginRealm");
	}
	...
	public void readingAnBladerunnerConfFileWithMissingValuesWillCauseAnException() throws Exception {
		given(brjs).containsFileWithContents("conf/bladerunner.conf", "defaultInputEncoding: UTF-8\njettyPort: 7070\nloginRealm: BladeRunnerLoginRealm");
		when(brjs).bladerunnerConf();
		then(exceptions).verifyException(ConfigException.class, brjs.file("conf/bladerunner.conf").getPath(), unquoted("'defaultOutputEncoding' may not be null"));
	}
	...
	public void invalidEncodingValuesWillCauseAnException() throws Exception {
		given(brjs).containsFileWithContents("conf/bladerunner.conf", "defaultInputEncoding: ZZZ-8\ndefaultOutputEncoding: UTF-8\njettyPort: 7070\nloginRealm: BladeRunnerLoginRealm");
		when(brjs).bladerunnerConf();
		then(exceptions).verifyException(ConfigException.class, brjs.file("conf/bladerunner.conf").getPath(), "defaultInputEncoding", "ZZZ-8",
	...
	public void readingAnBladerunnerConfFileWithEmptyValuesWillCauseAnException() throws Exception {
		...
		when(brjs).bladerunnerConf();
		then(exceptions).verifyException(ConfigException.class, brjs.file("conf/bladerunner.conf").getPath(), unquoted("'defaultOutputEncoding' may not be empty"));
	}
