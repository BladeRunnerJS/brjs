...
public class TypeScriptContentPlugin extends AbstractContentPlugin {
	...
	
	{
		try {
			...
			contentPathParserBuilder
				.accepts("typescript/bundle.js").as("bundle-request")
					.and("typescript/module/<module>.js").as("single-module-request")
	...
	public List<String> getValidDevContentPaths(BundleSet bundleSet, String locale) throws BundlerProcessingException {
		...
		transpileTypeScipt(bundleSet);
		
		try {
			for(SourceModule sourceModule : bundleSet.getSourceModules()) {
				if(sourceModule instanceof TypeScriptSourceModule) {
	...
	private void transpileTypeScipt(BundleSet bundleSet) throws BundlerProcessingException {
		try {
			List<String> commandArgs = new ArrayList<>();
			String requirePrefix = null;
			
			commandArgs.add("tsc");
			
			for(SourceModule sourceModule : bundleSet.getSourceModules()) {
				if (sourceModule instanceof TypeScriptSourceModule) {
					String nextRequirePrefix = sourceModule.getAssetLocation().requirePrefix();
					
					if((requirePrefix == null) || (nextRequirePrefix.length() < requirePrefix.length())) {
						requirePrefix = nextRequirePrefix;
					}
					
					commandArgs.add(sourceModule.getUnderlyingFile().getPath());
				}
			}
			
			if(requirePrefix != null) {
				File storageDir = bundleSet.getBundlableNode().getApp().storageDir("typescript");
				File outputDir = new File(storageDir, requirePrefix);
				
				commandArgs.add("--module");
				commandArgs.add("commonjs");
				commandArgs.add("--outDir");
				commandArgs.add(outputDir.getPath());
				
				CommandRunner.runCommand(brjs, commandArgs);
			}
		}
		catch(CommandOperationException | RequirePathException e) {
			throw new BundlerProcessingException(e);
		}
	}
}
